...
def write_to_fpga_register(fpga, register_dict):
    ...
    try:
        ...
        if not set( register_dict.keys() ).issubset( fpga.listdev() ):
            print "given register names are not included in the fpga devlist"
            return
...
def configure_downlink_registers(fpga, roachid):
    ...
    udp_src_mac =  config['udp_src_mac']
    udp_dest_mac = config['udp_dest_mac']

    udp_src_ip   = config['udp_src_ip']
