...
class valon5009(object):
    ...
    def referenceFrequency(self):
        ...
        if ret[2] == 'GHz':
            ...
            f*=1e3
        self._referenceFrequency = f
        return self._referenceFrequency

    @referenceFrequency.setter
    def referenceFrequency(self,value):
        assert (value >=5e6) and (value <= 100e6)
        self.sendCommand('ref %s'%value)

class valon5009Src(object):
    ...
    @property
    def frequency(self):
        ret = self.valonDevice.sendCommand('s%d; frequency?'%self.sourceNumber).split()
        f=float(ret[1])
        if ret[2] == 'GHz':
            f*=1e9
        elif ret[2] == 'MHz':
    ...
    @property
    def powerLevel(self):
        self._powerLevel = float(self.valonDevice.sendCommand('s%d; plevel?'%self.sourceNumber).split()[1][:-1])
