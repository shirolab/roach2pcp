...
hwid_snrs = ['AM01H05BA'] # probably xD
dev_snrs = ['whaterver'] # to modify when connection established
...
class valon5008:
    def __init__(self, device_serial_number=dev_snrs[0], timeout=1.0):
        print 'Connecting to Valon 5009 Synthesiser...'
        self.FREQMIN      = 137.5e6
        self.FREQMAX      = 4400e6
        self.powList      = [-4,-1,2,5]
        self.REFMIN       = 5e6
        self.REFMAX       = 150e6
        ...
        print 'OK :)'
        # self.conn = serial.Serial(None, 9600, serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE)
...
oo
    ...
    def set_ref_select(self, e_not_i = 1):
        ...
        """
        self.conn.open()
        bytes = struct.pack('>BB', 0x06, (not e_not_i) & 1)
        checksum = self._generate_checksum(bytes)
        try:
            self.conn.write(bytes + checksum)
            bytes = self.conn.read(1)
