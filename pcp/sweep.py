...
from .lib import lib_dirfiles as libdf
from .kid import resonator_routines as resfun
...
class pcpSweep(object):
    ...
    def exclude_idxs(self):
        return self.ip.exclude_idxs if self.ip is not None else []

    def get_new_f0s(self):
        return self.ip.get_updated_f0s() if self.ip is not None else []
    ...
    def load_sweep_dirfile(self, dirfile, auto_analyse = False):
        ...
        assert libdf.check_valid_sweep_dirfile(dirfile), "{0} doesn't appear to be a valid pcp sweep file".format(dirfile)

        dirfile = libdf.open_dirfile(dirfile) # opens if a path, passes df object if given
        #
        # if data exists, save a copy to the history
        if self.dirfile is not None and libdf.is_dirfile_valid(self.dirfile):
            _logger.info("previous data appears to exist - saved a copy of {0} to history".format(os.path.basename(self.dirfile.name)))
            #self.store_sweep()
    ...
    def get_data(self):
        """
        Function to return the data from the sweep dirfile and populate the class.
        ...
        sweep_data_fields = libdf.get_fields_in_fragment(self.dirfile, "format", exclude_index = True) # get the main fragment

        self.tonenames = np.array(self.dirfile.get_sarray( sweep_data_fields.pop(sweep_data_fields.index('tonenames') )))
        ...
        self.rf_freqs  = np.repeat(self._bb_freqs[:, np.newaxis], self.lo_freqs.shape[0], axis=1) + self.lo_freqs

        self.tonefreqs = self._bb_freqs + self._lo_freq
        #self.data_fields = [s.split(".")[-1] for s in sweep_data_fields]
        self.data        = np.array( [self.dirfile.get_carray(fc) for fc in self.tonenames] )

        #calparam_fields = filter(lambda x: x.startswith("calparam."), self.dirfile.entry_list())
        calparam_fields = libdf.get_fields_in_fragment( self.dirfile, 'calparam' )
        ...
        # self.calparams       =  np.array( [self.dirfile.get_carray(fc) for fc in cal_param_fields] )

        caldata_fields = libdf.get_fields_in_fragment( self.dirfile, 'caldata' )
        self.caldata_fields = [s.split(".")[-1] for s in caldata_fields]
        self.caldata        =  np.array( [self.dirfile.get_carray(fc) for fc in caldata_fields] )
        ...
        # self.caldata        =  np.array( [self.dirfile.get_carray(fc) for fc in cal_data_fields] )
    ...
    def despike(data, threshold):
        """ NOT IMPLEMENTED YET. Identify indices that are likely to be erroneous, by some rms threshold """

    def calc_sweep_cal_params(self, tonefreqs = None, method = "maxspeed", exclude_idxs=[], exclude_endpoints=2, choose_range=False,despike_window=None):
        """
        ...
        nan_mask = np.zeros_like(self.rf_freqs, dtype='bool')  # Must be a bool for this to work
        ...
        calparams, caldata = resfun.calc_sweep_cal_params( self.rf_freqs,
                                                            self.data.real,
                                                            self.data.imag,
                                                            tonefreqs,
                                                            nanmask = nan_mask,
        ...
        exclude_idxs = self.exclude_idxs if exclude_idxs == True else exclude_idxs

        allidxs = np.arange( len(self.data), dtype=np.int )
    ...
    def find_res_peaks(self, data, pm = None):
        ...
        """
        data = np.gradient(np.unwrap(np.angle(self.data)), axis=-1) # calculates the gradient for all resonators
        pm = pm if pm and len(pm) == 2 else np.std(data), np.ptp(data)
        peakidxs, pdict = sig.find_peaks(data, height = 0, width = 4, prominence = pm )

    def _write_sweep_cal_params_todisk(self, dirfile, calparams, caldata_dict):

        assert libdf.is_dirfile_valid(dirfile)
        ...
        is_calparamfrag_valid, calparamfrag = libdf.check_fragment_valid(dirfile, "calparam")
        is_caldatafrag_valid,  caldatafrag  = libdf.check_fragment_valid(dirfile, "caldata")
    ...
    def write_sweep_cal_params(self, overwrite = False):
        ...
        _, cal_frag_idx = libdf.check_fragment_valid(self.dirfile, "calparam")
        ...
        if not overwrite:
            ...
            shutil.copy( self.dirfile.fragment(cal_frag_idx).name, \
                        "_".join( (self.dirfile.fragment(cal_frag_idx).name, time.strftime(timestr_fmt))) \
                        )

        self._write_sweep_cal_params_todisk(self.dirfile, self.calparams, dict(zip(self.caldata_fields, self.caldata)) )
        #libdf.write_sweep_cal_params(cal_params, cal_data)
    ...
    def write_stream_cal(self, stream_dirfile):
        ...
        assert libdf.is_dirfile_valid(stream_dirfile)

        # calparams to write for dff0 conversion
        is_calfrag_valid, calfrag = libdf.check_fragment_valid(stream_dirfile, "calibration")
        assert is_calfrag_valid, "calibration fragment appears to be invalid"
        cal_ns = stream_dirfile.fragment(calfrag).namespace

        calfields = libdf.get_fields_in_fragment(stream_dirfile, 'calibration')
        assert set(calfields).issubset( libdf.DERIVED_CALPARAM_FIELDS ), "calfields don't appear to match what is expected; {0} - {1}".format( calfields, libdf.DERIVED_CALPARAM_FIELDS)

        # these need to be in the same order as libdf.DERIVED_CALPARAM_FIELDS
        ...
        for calfield, calval in zip(libdf.DERIVED_CALPARAM_FIELDS, caldatalist):
    ...
    def plot_sweep(self, sortfreqs = False, force_refresh = False):
        from pcp import visualisation as vis
        #sweeplist = [self].extend(self.history)

        if isinstance(self.ip, vis.pcpInteractivePlot) and not force_refresh:
            self.ip.reshow() # reshow the existing object
        else:
