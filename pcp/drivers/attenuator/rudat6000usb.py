...
RX_BUF_SIZE = 64 #bytes [B0,B1,B2...BN]i

##The following are now defined in the constructor
#ATT_MAX     = 60.0
#ATT_MIN     = 0.0
#RESOLUTION  = 0.25
...
class rudat6000Source(object):
	...
	def __init__(self, rudat6000Device, sourceNumber):
		...
		self.ATT_MAX = self.rudat6000Device.ATT_MAX
		self.ATT_MIN = self.rudat6000Device.ATT_MIN
		self.RESOLUTION = self.rudat6000Device.RESOLUTION
	...
	@att.setter
	def att(self,value):
		assert value <= self.ATT_MAX, 'Attenuation over range!'
		assert value >= self.ATT_MIN, 'Attenuation under range!'
		assert (value/self.RESOLUTION)%1==0, 'Resolution is %f dB!'%(self.RESOLUTION)
		integer_value    = int(value)
		fractional_value = int((value - integer_value)/self.RESOLUTION)
