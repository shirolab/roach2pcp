...
def read_tonelist(fname,lofreq = 0):
	...
	return np.genfromtxt(fname, skip_header=1, delimiter='\t', unpack=True)[1] - lofreq

def read_npy_target_and_bb_freqs(LOfreq, sweep_path):
    '''Given a directory path to sweep, imports the saved target freqs tfreqs (actual tones after mix up), and basebase 
    frequencies, bbfreqs the tones written to the fpga. target freqs are in no particular order, bbfreqs arranged correctly for python fft stuff'''
    tfreqs = np.load(os.path.join(sweep_path,'target_freqs.npy'))#in MHz
    bbfreqs = np.load(os.path.join(sweep_path,'bb_freqs.npy'))
    bbfreqs = bbfreqs[bbfreqs != 0]
    bbfreqs = np.roll(bbfreqs, - np.argmin(np.abs(bbfreqs)) - 1)
    return tfreqs,bbfreqs

# read in files
def importIQsweep(sweepdir,doplots=False):
    # read in corresponding sweeps
    sweepfs = np.array(sorted([os.path.join(sweepdir, f) for f in os.listdir(sweepdir) if 'I' in f or 'Q' in f]))
