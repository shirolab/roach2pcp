...
class roachInterface(object):
	...
	pos_freqs, self.pos_delta = np.linspace(self.min_pos_freq,
                                                self.max_pos_freq,
                                                self.Nfreq, retstep = True)
	neg_freqs, self.neg_delta = np.linspace(self.min_neg_freq + self.symm_offset,
                                                self.max_neg_freq + self.symm_offset,
                                                self.Nfreq, retstep = True)
	self.center_freq = np.float(self.gc['center_freq'])
        # Move these hard-coded numbers to config file?
